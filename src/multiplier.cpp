...
SC_MODULE(CORE){
            ...
            if (state.read() == 1) {
                ...
                switch(mode.read()) { 
                    ...
                    case 2:
                        ...
                        if (cnt_mult.read() == 12 && newstart.read()) {
                            cnt_mult.write(0);
                        }
                        else {
                            if (newstart.read()) {
        ...
        if (state.read() == 1) {
            switch(mode.read()) { 
                ...
                case 2: 
                    ...
                    if (cnt_mult.read() == 0 && mult_finish.read()) {
                        mult_clean.write(1);
    ...
    void proc() {
        switch(mode.read()) { 
            ...
            case 1: 
                ...
                break; 
            case 2:
                mult_start.write(start.read() || newstart.read());
                mult_p.write(p.read());
                mult_isMul.write(1);
                if (cnt_mult.read() == 0) {
                    mult_a.write(a.read()[cnt_a.read()]);
    ...
    SC_CTOR(CORE) : mult0("MULT0") {
        ...
        sensitive << rst_n;
        SC_METHOD(proc);
        sensitive << mode << start << a << b << p << cnt_a << cnt_mult << add_input_2nd << mult_finish << mult_out << sqr << c;
    }
};
